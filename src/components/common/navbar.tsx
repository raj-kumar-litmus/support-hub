...
const Navbar: FC<Props> = ({ showSidePane, showSidePaneGrid, setShowSidePaneGrid, openSearchField, setOpenSearchField }) => {
  ...
  return (
    <div className='py-4 pl-4 pr-1 sm:px-10 flex items-center border-slate-200  border-solid	 border-b justify-between h-16'>
      <div className='flex'>
        {showSidePane &&
          <CustomImage className="h-[13px] sm:hidden pr-4 cursor-pointer" src={MenuIcon} alt="Menu" onClick={() => setShowSidePaneGrid(!showSidePaneGrid)} />
        }
        <CustomImage className="h-[13px] cursor-pointer" src={SephoraLogo} alt="SEPHORA" />
      </div>
