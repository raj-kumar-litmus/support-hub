    ...
    <div className="rounded-lg border border-black-400 overflow-x-auto mt-4">
      <table className={`${props.tableClassName} w-full`}>
        {props.tableHead && (
          ...
          >
            <th colSpan={tableHeadColSpan} className="p-2"> {props.tableHead}</th>
          </thead>
        )}
        {props.tableData?.length > 0 &&
          props.tableData.map((row, index) => (
            <tr
              key={index}
              className={`text-sm ${index % 2 === 0 ? "bg-black-300" : "bg-black-100"
                }`}
            >
              {Object.entries(row).map(([key, value]: [string, string | number | Array<string | number>], i) => (
                <>
